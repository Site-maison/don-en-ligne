 ...
 * Soutien Model
 */
class SoutiensModelSoutien extends JModelAdmin
{
	...
	*/
	...
	//	$extension = $this->getState('soutien');
		...
		// Get the form.
		$form = $this->loadForm('com_soutiens.soutien', 'soutien', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_soutiens.edit.soutien.data', array());
		...
		$this->preprocessData('com_soutiens.soutien', $data);
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__soutiens_soutien');
				$max = $db->loadResult();
